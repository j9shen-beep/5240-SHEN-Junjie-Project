...
# Ensures all packages are available even if requirements.txt is not picked up
# by Streamlit Cloud's build environment.
# ─────────────────────────────────────────────────────────────────────────────
import subprocess
import sys
...
for pkg in required_packages:
    try:
        ...
        pass  # If already installed, continue silently

# ─────────────────────────────────────────────────────────────────────────────
# STANDARD IMPORTS  (after ensuring packages are installed)
# ─────────────────────────────────────────────────────────────────────────────
import re
import time
...
# PLOTLY GAUGE
# ─────────────────────────────────────────────────────────────────────────────
def make_gauge(index: float, signal: str, color: str) -> go.Figure:
    fig = go.Figure(
        go.Indicator(
            mode="gauge+number",
            value=index,
            title={
                "text": (
                    f"Dividend Danger Index<br>"
                    f"<span style='font-size:0.85em;color:{color}'>{signal}</span>"
                ),
                "font": {"size": 18},
            },
            number={"font": {"size": 52, "color": color}},
            gauge={
                "axis": {"range": [0, 100], "tickwidth": 1, "nticks": 11},
                ...
                "steps": [
                    ...
                    {"range": [30, 55],  "color": "#FAEEDA"},
                    {"range": [55, 75],  "color": "#FAECE7"},
...
def main():
    ...
    with g_col:
        st.plotly_chart(make_gauge(danger_index, signal, color), use_container_width=True)
    ...
    with t_col2:
        ...
        bar_colors = [
            "#A32D2D" if v > 0.6 else
            "#D85A30" if v > 0.4 else
            "#BA7517" if v > 0.25 else "#1D9E75"
            for v in vals
        ]
        tfig = go.Figure(
            go.Bar(x=vals, y=labels, orientation="h", marker_color=bar_colors)
        )
        tfig.update_layout(
            height=220,
